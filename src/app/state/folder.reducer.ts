...
import { addGradeSuccess, deleteGradeSuccess } from "./grade.action";
...
export const folderReducer = createReducer(
    ...
    on(deleteGradeSuccess, (state, subject) => state.map(folder => {folder = {...folder};folder.subjects = folder.subjects?.map(_subject => { if (_subject.id == subject.id) {console.log(subject);return subject}; return _subject }); return folder}))
